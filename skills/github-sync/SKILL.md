...
2. **Check config**: read `.task-manager.local.md` at repo root. If `projectId` key is present and non-empty → report `"already initialized (projectId: <id>)"` and **exit without any mutations**.

3. **Create project**: run `gh project create --owner @me --title "Task Manager"` (private by default). Capture the project number and node ID from output.
...
6. **Write config**: save all IDs to `.task-manager.local.md` (see Config File section).

7. **Update .gitignore**: append `.task-manager.local.md` to the repo root `.gitignore` if not already present.

## Config File

Location: `.task-manager.local.md` at **repo root** (not inside `.claude/`).

This file is human-readable with a YAML-style key:value block:

```
# Task Manager — Local Config
...
1. **Read config**: parse `.task-manager.local.md`. If file missing or `projectId` is empty/absent:
   ```
